...
<template>
  <div class="min-h-screen bg-gray-50 overflow-x-hidden">
    ...
    <div v-else-if="product" class="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
      <!-- Breadcrumb -->
      <nav class="mb-8 flex items-center gap-2 text-sm text-gray-600 overflow-hidden">
        <NuxtLink to="/" class="hover:text-gray-900 whitespace-nowrap">Home</NuxtLink>
        <svg class="h-4 w-4 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7" />
        </svg>
        <NuxtLink to="/products" class="hover:text-gray-900 whitespace-nowrap">Products</NuxtLink>
        <svg class="h-4 w-4 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7" />
        </svg>
        <span class="text-gray-900 truncate min-w-0" :title="product.title">{{ product.title }}</span>
      </nav>
      ...
      <div class="grid gap-6 lg:gap-8 lg:grid-cols-2 overflow-hidden">
        ...
        <div class="space-y-4 min-w-0">
          <!-- Main Image -->
          <div 
            class="aspect-square overflow-hidden rounded-lg bg-white shadow-lg cursor-zoom-in w-full"
            @click="openLightbox"
          >
            <img
              :src="selectedImage"
          ...
          <div v-if="allImages.length > 1" class="relative group">
            ...
            <button
              ...
              class="absolute left-0 top-10 -translate-y-1/2 z-10 bg-white/90 hover:bg-white shadow-lg rounded-full p-2 md:opacity-0 md:group-hover:opacity-100 transition-opacity"
            ...
            <div 
              ref="carouselRef"
              class="flex gap-2 overflow-x-auto pb-2 scroll-smooth scrollbar-thin scrollbar-thumb-gray-300 scrollbar-track-gray-100 snap-x snap-mandatory"
            >
              <button
            ...
            <button
              ...
              class="absolute right-0 top-10 -translate-y-1/2 z-10 bg-white/90 hover:bg-white shadow-lg rounded-full p-2 md:opacity-0 md:group-hover:opacity-100 transition-opacity"
        ...
        <div class="space-y-6 min-w-0">
          ...
          <!-- Title -->
          <h1 class="text-2xl sm:text-3xl font-bold text-gray-900 wrap-break-word">{{ product.title }}</h1>

          <!-- Description -->
          <p v-if="product.description" class="text-gray-600 wrap-break-word">
            {{ product.description }}
          </p>
          ...
          <div class="border-t border-b border-gray-200 py-6">
            <div class="flex flex-wrap items-baseline gap-2 sm:gap-3">
              ...
              <span v-if="product.original_price && product.original_price > product.current_price" class="text-lg sm:text-xl text-gray-500 line-through">
          ...
          <div class="space-y-3">
            <a
              ...
              class="flex w-full items-center justify-center gap-2 rounded-lg bg-blue-600 px-6 py-4 text-base sm:text-lg font-semibold text-white shadow-lg transition-all hover:bg-blue-700 hover:shadow-xl"
            >
          ...
          <!-- Marketplace Info -->
          <div v-if="product.marketplace" class="flex flex-col sm:flex-row items-start sm:items-center gap-2 rounded-lg bg-gray-100 p-4">
      ...
      <div v-if="product.metadata?.filters && Object.keys(product.metadata.filters).length > 0" class="mt-8">
        <div class="rounded-lg bg-linear-to-br from-blue-50 to-indigo-50 p-6 border border-blue-200">
          ...
          <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
            <!-- Chip Details -->
            <div v-if="product.metadata.filters.chip" class="bg-white rounded-lg p-4 shadow-sm">
              <div class="flex items-center gap-2 mb-1">
                <svg class="h-4 w-4 text-blue-600" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 3v2m6-2v2M9 19v2m6-2v2M5 9H3m2 6H3m18-6h-2m2 6h-2M7 19h10a2 2 0 002-2V7a2 2 0 00-2-2H7a2 2 0 00-2 2v10a2 2 0 002 2zM9 9h6v6H9V9z" />
                </svg>
                <span class="text-xs font-medium text-gray-500 uppercase">Chip Model</span>
              </div>
              <p class="text-sm font-semibold text-gray-900">{{ product.metadata.display?.chip || product.metadata.filters.chip }}</p>
            </div>

            <!-- PSRAM -->
            <div v-if="product.metadata.filters.psram_mb" class="bg-white rounded-lg p-4 shadow-sm">
              <div class="flex items-center gap-2 mb-1">
                <svg class="h-4 w-4 text-purple-600" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 12h14M5 12a2 2 0 01-2-2V6a2 2 0 012-2h14a2 2 0 012 2v4a2 2 0 01-2 2M5 12a2 2 0 00-2 2v4a2 2 0 002 2h14a2 2 0 002-2v-4a2 2 0 00-2-2m-2-4h.01M17 16h.01" />
                </svg>
                <span class="text-xs font-medium text-gray-500 uppercase">PSRAM</span>
              </div>
              <p class="text-sm font-semibold text-gray-900">{{ product.metadata.display?.psram || `${product.metadata.filters.psram_mb} MB` }}</p>
            </div>

            <!-- Flash Memory -->
            <div v-if="product.metadata.filters.flash_mb" class="bg-white rounded-lg p-4 shadow-sm">
              <div class="flex items-center gap-2 mb-1">
                <svg class="h-4 w-4 text-pink-600" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 12h14M5 12a2 2 0 01-2-2V6a2 2 0 012-2h14a2 2 0 012 2v4a2 2 0 01-2 2M5 12a2 2 0 00-2 2v4a2 2 0 002 2h14a2 2 0 002-2v-4a2 2 0 00-2-2m-2-4h.01M17 16h.01" />
                </svg>
                <span class="text-xs font-medium text-gray-500 uppercase">Flash Memory</span>
              </div>
              <p class="text-sm font-semibold text-gray-900">{{ product.metadata.display?.flash || `${product.metadata.filters.flash_mb} MB` }}</p>
            </div>

            <!-- GPIO Pins -->
            <div v-if="product.metadata.filters.gpio_pins" class="bg-white rounded-lg p-4 shadow-sm">
              <div class="flex items-center gap-2 mb-1">
                <svg class="h-4 w-4 text-gray-600" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6h16M4 12h16M4 18h16" />
                </svg>
                <span class="text-xs font-medium text-gray-500 uppercase">GPIO Pins</span>
              </div>
              <p class="text-sm font-semibold text-gray-900">{{ product.metadata.display?.gpio || `${product.metadata.filters.gpio_pins} pins` }}</p>
            </div>

            <!-- WiFi -->
            <div v-if="product.metadata.filters.has_wifi || product.metadata.filters.wifi_version" class="bg-white rounded-lg p-4 shadow-sm">
              <div class="flex items-center gap-2 mb-1">
                <svg class="h-4 w-4 text-green-600" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8.111 16.404a5.5 5.5 0 017.778 0M12 20h.01m-7.08-7.071c3.904-3.905 10.236-3.905 14.141 0M1.394 9.393c5.857-5.857 15.355-5.857 21.213 0" />
                </svg>
                <span class="text-xs font-medium text-gray-500 uppercase">WiFi</span>
              </div>
            ...
            <!-- Bluetooth -->
            <div v-if="product.metadata.filters.has_bluetooth || product.metadata.filters.bluetooth_version" class="bg-white rounded-lg p-4 shadow-sm">
              <div class="flex items-center gap-2 mb-1">
                <svg class="h-4 w-4 text-blue-600" fill="currentColor" viewBox="0 0 24 24">
                  <path d="M17.71 7.71L12 2h-1v7.59L6.41 5 5 6.41 10.59 12 5 17.59 6.41 19 11 14.41V22h1l5.71-5.71-4.3-4.29 4.3-4.29zM13 5.83l1.88 1.88L13 9.59V5.83zm1.88 10.46L13 18.17v-3.76l1.88 1.88z"/>
                </svg>
                <span class="text-xs font-medium text-gray-500 uppercase">Bluetooth</span>
              </div>
              <p class="text-sm font-semibold text-gray-900">
            ...
            <!-- Zigbee -->
            <div v-if="product.metadata.filters.has_zigbee" class="bg-white rounded-lg p-4 shadow-sm">
              <div class="flex items-center gap-2 mb-1">
                <svg class="h-4 w-4 text-yellow-600" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 10V3L4 14h7v7l9-11h-7z" />
                </svg>
                <span class="text-xs font-medium text-gray-500 uppercase">Zigbee</span>
              </div>
              <p class="text-sm font-semibold text-gray-900">Supported</p>
            </div>

            <!-- Thread -->
            <div v-if="product.metadata.filters.has_thread" class="bg-white rounded-lg p-4 shadow-sm">
              <div class="flex items-center gap-2 mb-1">
                <svg class="h-4 w-4 text-orange-600" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 10V3L4 14h7v7l9-11h-7z" />
                </svg>
                <span class="text-xs font-medium text-gray-500 uppercase">Thread</span>
              </div>
              <p class="text-sm font-semibold text-gray-900">Supported</p>
            </div>

            <!-- USB Type -->
            <div v-if="product.metadata.filters.usb_type" class="bg-white rounded-lg p-4 shadow-sm">
              <div class="flex items-center gap-2 mb-1">
                <svg class="h-4 w-4 text-slate-600" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 9l3 3-3 3m5 0h3M5 20h14a2 2 0 002-2V6a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" />
                </svg>
                <span class="text-xs font-medium text-gray-500 uppercase">USB Type</span>
              </div>
              <p class="text-sm font-semibold text-gray-900">{{ product.metadata.filters.usb_type.toUpperCase().replace('_', '-') }}</p>
            </div>

            <!-- Additional Features -->
            <div v-if="hardwareFeatures.length > 0" class="bg-white rounded-lg p-4 shadow-sm lg:col-span-3">
              <div class="flex items-center gap-2 mb-2">
                <svg class="h-4 w-4 text-indigo-600" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
                </svg>
                <span class="text-xs font-medium text-gray-500 uppercase">Additional Features</span>
              </div>
      ...
      <div v-if="product.features?.length || product.content_info || product.product_info || product.manufacture_info" class="mt-12">
        <h2 class="mb-6 text-2xl font-bold text-gray-900">Product Details</h2>
        
        <div class="grid gap-6 lg:grid-cols-2">
          <!-- Product Features List -->
          <div v-if="product.features && product.features.length > 0" class="rounded-lg bg-white p-6 shadow lg:col-span-2">
            ...
            <ul class="grid gap-2 md:grid-cols-2">
              ...
              >
                <svg class="h-5 w-5 mt-0.5 text-green-500 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          ...
          <!-- Content Information -->
          <div v-if="product.content_info && Object.keys(product.content_info).length > 0" class="rounded-lg bg-white p-6 shadow">
            <h3 class="mb-4 flex items-center gap-2 text-lg font-semibold text-gray-900">
              <svg class="h-5 w-5 text-blue-600" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <dl class="space-y-3">
              <div
                ...
                :key="idx"
                class="flex flex-col sm:flex-row sm:justify-between gap-1 sm:gap-2 border-b border-gray-100 pb-3 last:border-0 last:pb-0"
              >
                <dt class="font-medium text-gray-600">{{ key.replace(/([A-Z])/g, ' $1').trim() }}</dt>
                <dd class="text-gray-900 sm:text-right wrap-break-word">
          ...
          <!-- Product Information -->
          <div v-if="product.product_info && Object.keys(product.product_info).length > 0" class="rounded-lg bg-white p-6 shadow">
            <h3 class="mb-4 flex items-center gap-2 text-lg font-semibold text-gray-900">
              <svg class="h-5 w-5 text-blue-600" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          ...
          <div v-if="product.manufacture_info && Object.keys(product.manufacture_info).length > 0" class="rounded-lg bg-white p-6 shadow">
            <h3 class="mb-4 flex items-center gap-2 text-lg font-semibold text-gray-900">
              <svg class="h-5 w-5 text-blue-600" fill="none" viewBox="0 0 24 24" stroke="currentColor">
      ...
      <div v-if="product.customer_review_count || product.star_rating" class="mt-12">
        <h2 class="mb-6 text-2xl font-bold text-gray-900">Customer Reviews</h2>
        ...
        <div class="rounded-lg bg-white p-6 shadow">
          <div class="flex flex-col sm:flex-row items-start sm:items-center gap-4 sm:gap-6">
            ...
            <div v-if="product.star_rating" class="text-center">
              <div class="flex items-center gap-2 mb-2">
                <span class="text-4xl sm:text-5xl font-bold text-gray-900">{{ product.star_rating }}</span>
              ...
              <div class="flex items-center gap-1">
                <svg v-for="i in 5" :key="i" 
                  class="h-5 w-5" 
                  :class="i <= Math.round(product.star_rating) ? 'text-yellow-400' : 'text-gray-300'"
                  fill="currentColor" 
                  viewBox="0 0 24 24"
                >
                  <path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z"/>
            ...
            <!-- Review Count -->
            <div v-if="product.customer_review_count" class="sm:border-l border-gray-200 sm:pl-6">
              <p class="text-2xl sm:text-3xl font-bold text-gray-900">{{ product.customer_review_count.toLocaleString() }}</p>
            ...
            <div class="sm:ml-auto w-full sm:w-auto">
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                class="inline-flex w-full sm:w-auto items-center justify-center gap-2 rounded-lg bg-gray-100 px-4 py-2 text-sm font-medium text-gray-700 transition-colors hover:bg-gray-200"
              >
